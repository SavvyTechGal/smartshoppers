...
@Component({
  ...
  styleUrls: ['./casual-quiz.component.css']
})
export class CasualQuizComponent implements OnInit {
  userEmail: string = '';
  ...
  public AnswersObject: any = {  //will hold edited responses onSubmit
    ...
    brands:[],
    brandRank:'',
    streamsMovies:'no', //unique to this version -- START
    streamsMoviesRank:'',
    workUse:'no',
    workUseRank:'',
    sensitiveInfo:'no',
    sensitiveInfoRank:'',
    streamsMusic:'no',
    streamsMusicRank:'', ////unique to this version -- END
    storage: 'no',
    storageRank: '',
    charges: 'no',
    chargesRank: '',
  ...
  onSubmit(quizanswers: { value: any; }) //takes in all answers (checkbox and dropdown values) from the questionnaire
  {
    ...
    this.AnswersObject.streamsMoviesRank = (quizanswers.value.streamsMoviesRank).toString();
    
    if(quizanswers.value.streamsMusic) {
      this.AnswersObject.streamsMusic = 'yes';
    }
    this.AnswersObject.streamsMusicRank = (quizanswers.value.streamsMusicRank).toString();

    if(quizanswers.value.workUse) {
      this.AnswersObject.workUse = 'yes';
    }
    this.AnswersObject.workUseRank = (quizanswers.value.workUseRank).toString();

    if(quizanswers.value.sensitiveInfo) {
      this.AnswersObject.sensitiveInfo = 'yes';
    }
    this.AnswersObject.sensitiveInfoRank = (quizanswers.value.sensitiveInfoRank).toString();
    ...
    // this.answerService.postAnswer(this.userEmail, '18', this.AnswersObject.streamsMusic, this.AnswersObject.streamsMusicRank);
    // this.answerService.postAnswer(this.userEmail, '19', this.AnswersObject.workUse, this.AnswersObject.workUseRank);
    // this.answerService.postAnswer(this.userEmail, '20', this.AnswersObject.sensitiveInfo, this.AnswersObject.sensitiveInfoRank);
